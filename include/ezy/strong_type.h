...
class strong_type : public Features<strong_type<T, Tag, Features...>>...
{
  public:
    ...
    explicit strong_type(T&& value,
        //std::enable_if_t<!std::is_lvalue_reference_v<T>>* = nullptr // disable if underlying type is an lvalue ref
        detail::disable_if_t<std::is_lvalue_reference_v<T>>* = nullptr // disable if underlying type is an lvalue ref
    ...
    explicit strong_type(Args&&... args
        //, std::enable_if_t<std::is_constructible_v<type, Args...>>
