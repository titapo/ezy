...
namespace ezy
{
  ...
  {
    template <typename T>
    constexpr decltype(auto) dependent_forward_impl(T&& t, ezy::experimental::owner_category_tag, std::false_type)
    { return std::move(t); }

    template <typename T>
    constexpr decltype(auto) dependent_forward_impl(T&& t, ezy::experimental::reference_category_tag, std::false_type)
    { return t; }

    // TODO const cases are missing

    template <typename Dependence, typename T>
    constexpr decltype(auto) dependent_forward(T&& t)
    {
